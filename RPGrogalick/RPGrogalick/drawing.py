...
class Drawing:
    def __init__(self, sc, sc_map):
        ...
        self.textures = {
            ...
            'staircase': pygame.image.load("image/ladder.png").convert(),  # Текстура лестницы
            ...
            # МЕЧ
            'sword': pygame.image.load("sprites_txt/sword_2.png").convert_alpha(),
            # ПРИЦЕЛ
    ...
    def enemy_counter(self, enemy_count):
        """Отображает счетчик врагов в левом верхнем углу"""
        font = pygame.font.SysFont('Arial', 28, bold=True)
        text = font.render(f"Enemies left: {enemy_count}", True, (255, 0, 0))
        text_shadow = font.render(f"Enemies left: {enemy_count}", True, (0, 0, 0))
        # Тень для лучшей читаемости
        self.sc.blit(text_shadow, (12, 12))
    ...
    def weapon(self, is_attacking):
        ...
        # Анимация атаки
        if is_attacking:
            self.sword_animation = min(self.sword_animation + 8, 30)  # Более быстрое движение
        else:
            # Плавный возврат к исходному положению
            self.sword_animation = max(self.sword_animation - 4, 0)
        ...
        # Смещение при атаке
        offset_y = -self.sword_animation * -4
        rotation = -self.sword_animation * -1.5  # Усиливаем вращение
        ...
        sword_x = width - sword_width // 2 + 50 - sword_rect.width // 2
        sword_y = height - sword_height // 2 + 100 + offset_y - sword_rect.height // 2
    ...
    def staircase_hint(self, can_use):
        """Отображает подсказку для взаимодействия с лестницей"""
        font = pygame.font.SysFont('Arial', 34, bold=True)

        if can_use:
            text = font.render('Press E to climb stairs', True, green)  # Зеленый цвет
        else:
            text = font.render('Defeat all enemies to use stairs', True, red)  # Красный цвет

        text_rect = text.get_rect(center=(half_width, height - 50))
    ...
    def enemy_counter(self, enemy_count):
        ...
        if enemy_count == 0:
            ...
            text_color = red
        font = pygame.font.SysFont('Arial', 28, bold=True)
        text = font.render(f"Enemies left: {enemy_count}", True, text_color)
        text_shadow = font.render(f"Enemies left: {enemy_count}", True, black)
        # Тень для лучшей читаемости
        self.sc.blit(text_shadow, (12, 102))
        self.sc.blit(text, (10, 100))

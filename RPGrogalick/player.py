...
class Player:
    def __init__(self):
        ...
        self.lava_damage = 5
        self.is_attacking = False  # НОВОЕ
        self.attack_cooldown = 0  # НОВОЕ
        self.is_dead = False  # НОВОЕ: флаг смерти
    ...
    #     self.attack_cooldown = 20  # Кулдаун атаки
    #
    ...
    #             # Проверяем угол (враг должен быть перед игроком)
    ...
    #                 angle_diff += 2 * math.pi
    #
    #             if abs(angle_diff) < math.pi / 3:  # 60 градусов впереди
    #                 killed = enemy.take_damage(attack_damage)
    #                 if killed:
    #                     print(f"Враг убит!")
    #                 else:
    #                     print(f"Удар! Здоровье врага: {enemy.health}")
